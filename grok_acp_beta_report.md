...
**Adversary check:** local IR inspection and lldb contradicted the guard-only framing: old generated `run_command_capture_output` had no parent/child branch before `execvp`, manual `llc` succeeded on the same `.ll`, and `ADAMAS_LLVM_CACHE=0` plus raw fork/wait fixes produced and ran a no-prelude `puts 7` binary.
