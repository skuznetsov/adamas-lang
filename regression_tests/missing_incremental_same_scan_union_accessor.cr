...
puts "[SAME_SCAN_ACCESSOR] before_getter_body=#{converter.module.has_function_with_body?(getter_target) ? 1 : 0}"
ENV["ADAMAS_MISSING_INCREMENTAL_FALSIFIER"] = "1"
converter.__probe_lower_missing_call_targets
puts "[SAME_SCAN_ACCESSOR] after_getter_body=#{converter.module.has_function_with_body?(getter_target) ? 1 : 0} second_scan_body=#{converter.module.has_function_with_body?(second_scan_target) ? 1 : 0}"
