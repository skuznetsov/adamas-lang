#!/usr/bin/env bash
# H6-core contract guard: type-visible values should share one semantic identity
# across typeof, runtime .class, direct output, interpolation, and type-literal
# name/string queries without relying on the parser-confounded no-parens
# `puts (expr).class` command-call shape.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
COMPILER="${1:-$ROOT_DIR/bin/adamas}"
ORIGINAL_CRYSTAL="${ORIGINAL_CRYSTAL:-crystal}"
TIMEOUT_SECS="${TYPEVALUE_CORE_ORACLE_TIMEOUT:-5}"
MAX_MEM_MB="${TYPEVALUE_CORE_ORACLE_MAX_MEM:-512}"
EXPECT_MISMATCH="${ADAMAS_EXPECT_TYPEVALUE_CORE_MISMATCH:-0}"
...
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/type_value_core_runtime_identity.XXXXXX")"
trap 'rm -rf "$WORKDIR"' EXIT

SRC="$WORKDIR/type_value_core.cr"
...
puts "LOCAL_NILABLE_CLASS"
puts nilable.class
puts "INTERP_LOCAL_NILABLE_CLASS=#{nilable.class}"

puts "PAREN_NILABLE_CLASS"
...
if ! "$COMPILER" "$SRC" -o "$STAGE_BIN" >"$STAGE_COMPILE_LOG" 2>&1; then
  echo "type-value core runtime identity oracle: stage compiler failed" >&2
  tail -n 40 "$STAGE_COMPILE_LOG" >&2
  if [[ "$EXPECT_MISMATCH" == "1" ]]; then
    echo "MEASURED_RED: expected TypeValue-core mismatch via stage compile failure"
    exit 0
  fi
  exit 1
fi
...
if [[ $stage_run_rc -ne 0 ]]; then
  echo "type-value core runtime identity oracle: stage binary failed (rc=$stage_run_rc)" >&2
  tail -n 80 "$STAGE_RUN_LOG" >&2 || true
  if [[ "$EXPECT_MISMATCH" == "1" ]]; then
    echo "MEASURED_RED: expected TypeValue-core mismatch via stage runtime failure"
    exit 0
  fi
  exit 1
fi
...
if diff -u "$ORIGINAL_STDOUT" "$STAGE_STDOUT" >"$WORKDIR/stdout.diff"; then
  if [[ "$EXPECT_MISMATCH" == "1" ]]; then
    echo "expected TypeValue-core runtime identity mismatch, but outputs matched" >&2
    cat "$ORIGINAL_STDOUT" >&2
    exit 1
  fi
  echo "PASS: TypeValue-core runtime identity oracle"
  exit 0
fi

echo "TypeValue-core runtime identity oracle mismatch" >&2
cat "$WORKDIR/stdout.diff" >&2

if [[ "$EXPECT_MISMATCH" == "1" ]]; then
  echo "MEASURED_RED: TypeValue-core runtime identity mismatch reproduced"
  exit 0
fi
