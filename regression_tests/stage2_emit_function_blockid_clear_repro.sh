...
set +e
STAGE2_DEBUG=1 ADAMAS_MIR_SETUP_TRACE=1 ADAMAS_LLVM_SETUP_TRACE=1 \
  "$COMPILER" "$SRC" --no-prelude --no-link -o "$BIN" >"$OUT" 2>"$ERR"
compile_status=$?
set -e

cat >"$LLDB_CMDS" <<LL
settings set target.env-vars STAGE2_DEBUG=1 ADAMAS_MIR_SETUP_TRACE=1 ADAMAS_LLVM_SETUP_TRACE=1
