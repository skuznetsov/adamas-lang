#!/usr/bin/env bash
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "usage: $0 <stage1-compiler> <stage2-compiler>" >&2
  exit 2
fi

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$ROOT_DIR/regression_tests/stage2_absolute_path_type_literal_hir_repro.cr"
STAGE1="$1"
STAGE2="$2"
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/stage2_absolute_path_type_literal_hir_repro.XXXXXX")"

cleanup() {
  rm -rf "$WORKDIR"
}
trap cleanup EXIT

for compiler in "$STAGE1" "$STAGE2"; do
  if [[ ! -x "$compiler" ]]; then
    echo "error: compiler binary not found/executable: $compiler" >&2
    exit 2
  fi
done

run_case() {
  local label="$1"
  local compiler="$2"
  local out_base="$WORKDIR/$label"
  local wrapper="$WORKDIR/$label.sh"
  local log="$WORKDIR/$label.log"

  {
    echo "#!/usr/bin/env bash"
    echo "set -euo pipefail"
    echo "export ADAMAS_STOP_AFTER_HIR=1"
    if [[ "$label" == "stage2" ]]; then
      echo "export ADAMAS_TRUST_SLICE_ADDR=1"
    fi
    printf 'exec %q ' "$compiler"
    printf '%q ' "$SRC" --release --no-prelude --no-ast-cache -o "$out_base"
    echo
  } >"$wrapper"
  chmod +x "$wrapper"

  set +e
  "$ROOT_DIR/scripts/run_safe.sh" "$wrapper" 15 1024 >"$log" 2>&1
  local status=$?
  set -e

  if [[ "$label" == "stage1" && $status -ne 0 ]]; then
    echo "inconclusive: stage1 failed during absolute-path type-literal HIR repro" >&2
    tail -n 80 "$log" >&2 || true
    exit 2
  fi

  if [[ "$label" == "stage2" && $status -ne 0 ]]; then
    echo "reproduced: stage2 failed during absolute-path type-literal HIR repro"
    tail -n 80 "$log"
    exit 1
  fi
}

run_case "stage1" "$STAGE1"
run_case "stage2" "$STAGE2"

echo "not reproduced: stage2 survives absolute-path type-literal HIR repro"
