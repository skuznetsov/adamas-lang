...
SRC="$ROOT_DIR/regression_tests/stage2_u64_literal_mir_oracle.cr"
STAGE1="$1"
STAGE2="$2"
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/stage2_u64_literal_mir.XXXXXX")"
...
run_phase() {
  ...
  if [[ $rc -ne 0 ]]; then
    if [[ $label == stage1 ]]; then
      echo "inconclusive: stage1 failed on large-u64 MIR oracle" >&2
      tail -n 80 "$log" >&2 || true
      exit 2
    fi
    echo "reproduced: self-hosted stage2 failed before MIR artifact on large UInt64 literals"
    tail -n 80 "$log"
    exit 1
  fi
  ...
  for expected in \
    "const 9223372036854775808 : UInt64" \
    "const 18446744073709551615 : UInt64"
  do
    if ! grep -Fq "$expected" "$artifact"; then
      if [[ $label == stage1 ]]; then
        echo "inconclusive: stage1 MIR missing expected large UInt64 constant" >&2
        echo "expected: $expected" >&2
        sed -n '1,120p' "$artifact" >&2
        exit 2
      fi
      echo "reproduced: self-hosted stage2 MIR lost expected large UInt64 constant"
      echo "expected: $expected"
      sed -n '1,120p' "$artifact"
      exit 1
    fi
  done
}
...
if ! diff -u "$WORKDIR/stage1.mir" "$WORKDIR/stage2.mir" >"$WORKDIR/mir.diff"; then
  echo "reproduced: stage2 diverges from stage1 in MIR for large UInt64 literals"
  sed -n '1,120p' "$WORKDIR/mir.diff"
  exit 1
fi

echo "not reproduced: stage2 matches stage1 MIR on large UInt64 literals"
