...
STAGE1="$1"
STAGE2="$2"
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/stage2_abstract_macro_char_literal.XXXXXX")"
...
ABSTRACT_SRC="$ROOT_DIR/regression_tests/stage2_abstract_macro_char_literal_parse_repro.cr"
...
if ! run_parse_only "$STAGE1" "$ABSTRACT_SRC" "stage1_abstract"; then
  echo "error: stage1 control unexpectedly failed on abstract char-literal reducer" >&2
  tail -n 40 "$WORKDIR/stage1_abstract.log" >&2
  exit 2
fi

if ! run_parse_only "$STAGE2" "$STRUCT_SRC" "stage2_struct"; then
  echo "error: stage2 struct control unexpectedly failed on char-literal reducer" >&2
  tail -n 40 "$WORKDIR/stage2_struct.log" >&2
  exit 2
fi

if run_parse_only "$STAGE2" "$ABSTRACT_SRC" "stage2_abstract"; then
  echo "not reproduced: stage2 now parses abstract macro char-literal reducer"
  exit 0
fi

echo "reproduced: stage2 still fails on abstract macro char-literal reducer while stage1 and struct control pass"
