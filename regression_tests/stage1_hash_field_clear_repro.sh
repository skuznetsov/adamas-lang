...
if [[ $RUN_STATUS -eq 0 ]]; then
  echo "not reproduced: object-field Hash clear/reuse preserves semantics"
  exit 0
fi

if rg -q '\[EXIT: 1\]' "$RUN_LOG"; then
  echo "reproduced: object-field Hash clear/reuse returned the wrong result"
  tail -n 80 "$RUN_LOG"
  exit 1
fi

echo "reproduced: object-field Hash clear/reuse crashed"
tail -n 80 "$RUN_LOG"
exit 1
