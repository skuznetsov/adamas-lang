...
STAGE1="$1"
STAGE2="$2"
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/stage2_abstract_macro_char_literal_oracle.XXXXXX")"
...
run_phase() {
  ...
  if [[ $compiler_label == "stage2" && $status -ne 0 ]]; then
    echo "reproduced: stage2 failed during ${phase} on the abstract macro char literal oracle"
    tail -n 80 "$log"
    exit 1
  fi
...
compare_phase() {
  ...
  if ! diff -u "$stage1_artifact" "$stage2_artifact" >"$diff_file"; then
    echo "reproduced: stage2 diverges from stage1 in ${phase} for the abstract macro char literal oracle"
...
echo "not reproduced: stage2 matches stage1 on hir/mir/ll for the abstract macro char literal oracle"
