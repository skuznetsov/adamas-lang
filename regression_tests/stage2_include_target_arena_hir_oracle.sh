...
SRC="$ROOT_DIR/regression_tests/stage2_include_target_arena_hir_oracle.cr"
STAGE1="$1"
STAGE2="$2"
WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/stage2_include_target_arena_hir_oracle.XXXXXX")"
...
run_hir() {
  ...
  if [[ $label == "stage1" && $status -ne 0 ]]; then
    echo "inconclusive: stage1 failed during include-target arena HIR oracle" >&2
    tail -n 80 "$log" >&2 || true
    exit 2
  fi

  if [[ $label == "stage2" && $status -ne 0 ]]; then
    echo "reproduced: stage2 failed before completing include-target arena HIR emission"
    tail -n 80 "$log"
    exit 1
  fi
...
if ! diff -u "$WORKDIR/stage1.hir" "$WORKDIR/stage2.hir" >"$WORKDIR/hir.diff"; then
  echo "reproduced: stage2 diverges from stage1 on the include-target arena HIR oracle"
  sed -n '1,120p' "$WORKDIR/hir.diff"
  exit 1
fi

echo "not reproduced: stage2 matches stage1 on the include-target arena HIR oracle"
