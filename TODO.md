...
- **Current focused slice**:
  ...
  - fresh re-localization on `/Users/sergey/Projects/Crystal/.codex_artifacts/stage2_release_tokenpreload_w1` splits the remaining work into two distinct later corridors:
    - `src/stdlib/object.cr --release --no-prelude` no longer stops in `parse_method_params`; batch LLDB now stops at `Hash(String, String)#resize -> StringPool#intern -> Parser#parse_identifier_like -> parse_expression -> parse_op_assign -> parse_def`, and a tiny standalone `Hash(String, String)` resize runtime probe compiled by the same stage1 compiler stays green (`1234`), so this looks like compiler-local corruption reaching `StringPool`/hash resize rather than a generic runtime hash failure
    - `src/crystal_v2.cr --release` parse-only is now a later heisenbug corridor instead of a hard early red: direct one-shot runs under `CRYSTAL_V2_STOP_AFTER_PARSE=1` yielded `[0, 0, 139]`, batch LLDB on a red run stops at `VirtualArena#[](ExprId) -> AstToHir#with_resolved_body_arena -> register_lib`, and `FORCE_LIB_REPARSE=1` only shifts that to another unstable series (`[139, 139, 0]`) instead of removing it
  - fresh refutation: a layout-changing arena-instrumentation branch is not trustworthy for diagnosing the current frontier. The temporary pair `/Users/sergey/Projects/Crystal/.codex_artifacts/stage1_release_libarena_trace_w1` (`~454s`) and `/Users/sergey/Projects/Crystal/.codex_artifacts/stage2_release_libarena_trace_w1` (`~158s`) built cleanly, but direct LLDB on that stage2 compiling `src/crystal_v2.cr --release` moved the first visible failure back to parser code (`Parser#parse_block -> attach_block_to_call -> parse_expression -> parse_op_assign -> parse_def -> parse_class -> parse_program_roots_impl -> CLI#parse_file_recursive`). Treat type/layout-tightening debug experiments as observer effects unless the same failure surface survives on a layout-neutral diff.
  - current handoff checkpoint keeps only a layout-neutral diagnostic hook in `src/compiler/cli.cr`: `DEBUG_LIB_ARENA` now logs the `LibEntry` arena right before `hir_converter.register_lib(...)`. Next step for the handoff agent is to rebuild a clean release pair on this minimal diff, run `DEBUG_LIB_ARENA=LibC` (or another tight filter) on `src/crystal_v2.cr --release`, and compare the CLI-side `entry.arena` with the previously observed late `register_lib` / `with_resolved_body_arena` LLDB corridor before attempting any fix
