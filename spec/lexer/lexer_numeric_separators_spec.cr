...
describe "Adamas::Compiler::Frontend::Parser" do
  describe "Phase 55: Numeric separators (underscores in numbers) (PRODUCTION-READY)" do
    it "parses decimal with thousand separator" do
      ...
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("1_000")
    ...
    it "parses decimal with million separator" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("1_000_000")
    end
    ...
    it "parses hex with separators" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("0xFF_FF")
    end
    ...
    it "parses hex with multiple separators" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("0x1A_2B_3C_4D")
    end
    ...
    it "parses binary with separators" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("0b1111_0000")
    end
    ...
    it "parses binary with byte separators" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("0b1010_1010_0101_0101")
    end
    ...
    it "parses octal with separators" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("0o777_666")
    end
    ...
    it "parses float with separators in integer part" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("1_234.56")
      Adamas::Compiler::Frontend.node_number_kind(number).should eq(Adamas::Compiler::Frontend::NumberKind::F64)
    end
    ...
    it "parses float with separators in fractional part" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("3.14_159_265")
      Adamas::Compiler::Frontend.node_number_kind(number).should eq(Adamas::Compiler::Frontend::NumberKind::F64)
    end
    ...
    it "parses float with separators in both parts" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("1_234.567_890")
    end
    ...
    it "parses number with suffix and separators" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("1_000_000_i64")
    ...
    it "parses hex with suffix and separators" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("0xDEAD_BEEF_i64")
      Adamas::Compiler::Frontend.node_number_kind(number).should eq(Adamas::Compiler::Frontend::NumberKind::I64)
    end
    ...
    it "parses numbers with separators in array" do
      ...
      String.new(Adamas::Compiler::Frontend.node_literal(num1).not_nil!).should eq("1_000")

      num2 = arena[elements[1]]
      String.new(Adamas::Compiler::Frontend.node_literal(num2).not_nil!).should eq("2_000")

      num3 = arena[elements[2]]
      String.new(Adamas::Compiler::Frontend.node_literal(num3).not_nil!).should eq("3_000")
    end
    ...
    it "parses number without separator (backward compatibility)" do
      ...
      assign = arena[program.roots[0]]
      number = arena[Adamas::Compiler::Frontend.node_assign_value(assign).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(number).not_nil!).should eq("1000")
    end
    ...
    it "parses binary expression with separators" do
      ...
      String.new(Adamas::Compiler::Frontend.node_literal(left).not_nil!).should eq("1_000")

      right = arena[Adamas::Compiler::Frontend.node_right(binary).not_nil!]
      String.new(Adamas::Compiler::Frontend.node_literal(right).not_nil!).should eq("2_000")
    end
  end
end
