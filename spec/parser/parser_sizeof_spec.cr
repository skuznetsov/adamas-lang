...
describe "Adamas::Compiler::Frontend::Parser" do
  describe "Phase 41: sizeof (size in bytes) (PRODUCTION-READY)" do
    it "parses sizeof with type identifier" do
      ...
      sizeof_expr = Adamas::Compiler::Frontend.node_assign_value(assign_node).not_nil!
      sizeof_node = arena[sizeof_expr]
      Adamas::Compiler::Frontend.node_kind(sizeof_node).should eq(Adamas::Compiler::Frontend::NodeKind::Sizeof)

      # Check arguments
      args = Adamas::Compiler::Frontend.node_sizeof_args(sizeof_node).not_nil!
      ...
      Adamas::Compiler::Frontend.node_literal(arg_node).not_nil!.should eq("Int32".to_slice)
    end
    ...
    it "parses nested sizeof" do
      ...
      assign_node = arena[program.roots[0]]
      outer_sizeof_expr = Adamas::Compiler::Frontend.node_assign_value(assign_node).not_nil!
      outer_sizeof = arena[outer_sizeof_expr]

      Adamas::Compiler::Frontend.node_kind(outer_sizeof).should eq(Adamas::Compiler::Frontend::NodeKind::Sizeof)

      # Outer sizeof has one argument
      outer_args = Adamas::Compiler::Frontend.node_sizeof_args(outer_sizeof).not_nil!
      outer_args.size.should eq(1)

      # That argument is also a sizeof
      inner_sizeof = arena[outer_args[0]]
      Adamas::Compiler::Frontend.node_kind(inner_sizeof).should eq(Adamas::Compiler::Frontend::NodeKind::Sizeof)

      # Inner sizeof has one argument (Int32)
      inner_args = Adamas::Compiler::Frontend.node_sizeof_args(inner_sizeof).not_nil!
