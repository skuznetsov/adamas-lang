...
describe "Adamas::Compiler::Frontend::Parser" do
  describe "Phase 70: Named tuple literals {name: \"value\"} (PRODUCTION-READY)" do
    it "parses simple named tuple with one entry" do
      ...
      Adamas::Compiler::Frontend.node_kind(named_tuple).should eq(Adamas::Compiler::Frontend::NodeKind::NamedTupleLiteral)

      entries = Adamas::Compiler::Frontend.node_named_tuple_entries(named_tuple).not_nil!
    ...
    it "parses named tuple with expression values" do
      ...
      Adamas::Compiler::Frontend.node_kind(value1).should eq(Adamas::Compiler::Frontend::NodeKind::Binary)
    end
    ...
    it "parses named tuple with identifier values" do
      ...
      value1 = arena[entries[0].value]
      Adamas::Compiler::Frontend.node_kind(value1).should eq(Adamas::Compiler::Frontend::NodeKind::Identifier)
    end
    ...
    it "parses nested named tuples" do
      ...
      Adamas::Compiler::Frontend.node_kind(outer).should eq(Adamas::Compiler::Frontend::NodeKind::NamedTupleLiteral)

      entries = Adamas::Compiler::Frontend.node_named_tuple_entries(outer).not_nil!
      entries.size.should eq(1)
      entries[0].key.should eq("person".to_slice)

      # Inner is also named tuple
      inner = arena[entries[0].value]
      Adamas::Compiler::Frontend.node_kind(inner).should eq(Adamas::Compiler::Frontend::NodeKind::NamedTupleLiteral)
    end
    ...
    it "parses named tuple in assignment" do
      ...
      Adamas::Compiler::Frontend.node_kind(value).should eq(Adamas::Compiler::Frontend::NodeKind::NamedTupleLiteral)
    end
    ...
    it "parses named tuple in method call" do
      ...
      arg = arena[args[0]]
      Adamas::Compiler::Frontend.node_kind(arg).should eq(Adamas::Compiler::Frontend::NodeKind::NamedTupleLiteral)
    end
    ...
    it "parses named tuple in array literal" do
      ...
      array_elements = Adamas::Compiler::Frontend.node_array_elements(array).not_nil!
      array_elements.size.should eq(2)

      # Both elements are named tuples
      elem1 = arena[array_elements[0]]
      Adamas::Compiler::Frontend.node_kind(elem1).should eq(Adamas::Compiler::Frontend::NodeKind::NamedTupleLiteral)

      elem2 = arena[array_elements[1]]
      Adamas::Compiler::Frontend.node_kind(elem2).should eq(Adamas::Compiler::Frontend::NodeKind::NamedTupleLiteral)
    end
    ...
    it "disambiguates named tuple from hash (colon vs arrow)" do
      ...
      Adamas::Compiler::Frontend.node_kind(node).should eq(Adamas::Compiler::Frontend::NodeKind::NamedTupleLiteral)
    end
    ...
    it "disambiguates hash from named tuple (arrow)" do
      ...
      node = arena[program.roots[0]]
      Adamas::Compiler::Frontend.node_kind(node).should eq(Adamas::Compiler::Frontend::NodeKind::HashLiteral)
    end
    ...
    it "disambiguates tuple from named tuple (comma vs colon)" do
      ...
      node = arena[program.roots[0]]
      Adamas::Compiler::Frontend.node_kind(node).should eq(Adamas::Compiler::Frontend::NodeKind::TupleLiteral)
    end
    ...
    it "parses hash literal with multiple arrows and chained calls" do
      ...
      hash = arena[Adamas::Compiler::Frontend.node_constant_value(const_node).not_nil!]
      Adamas::Compiler::Frontend.node_kind(hash).should eq(Adamas::Compiler::Frontend::NodeKind::HashLiteral)
    end
  end
end
