...
describe "Ternary Operator Edge Cases" do
  it "distinguishes ? in ternary from ? in method names" do
    ...
    ternary_id = Adamas::Compiler::Frontend.node_assign_value(assign_node).not_nil!
  ...
  it "handles : in ternary without conflicting with symbols" do
    ...
    true_id = Adamas::Compiler::Frontend.node_ternary_true_branch(ternary_node).not_nil!
    true_node = program.arena[true_id]
    Adamas::Compiler::Frontend.node_kind(true_node).should eq(Adamas::Compiler::Frontend::NodeKind::Symbol)

    # False branch should be symbol
    false_id = Adamas::Compiler::Frontend.node_ternary_false_branch(ternary_node).not_nil!
    false_node = program.arena[false_id]
    Adamas::Compiler::Frontend.node_kind(false_node).should eq(Adamas::Compiler::Frontend::NodeKind::Symbol)
  end
