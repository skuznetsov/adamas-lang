...
describe "Adamas::Compiler::Frontend::Parser" do
  describe "Phase 26: Modifier if/unless" do
    it "parses postfix if modifier with return" do
      ...
      then_body = Adamas::Compiler::Frontend.node_if_then(if_node).not_nil!
      then_body.size.should eq(1)

      return_stmt = arena[then_body[0]]
      Adamas::Compiler::Frontend.node_kind(return_stmt).should eq(Adamas::Compiler::Frontend::NodeKind::Return)
    end
    ...
    it "parses postfix if modifier with break" do
      ...
      Adamas::Compiler::Frontend.node_kind(break_stmt).should eq(Adamas::Compiler::Frontend::NodeKind::Break)
    end
    ...
    it "parses postfix unless modifier with next" do
      ...
      Adamas::Compiler::Frontend.node_kind(next_stmt).should eq(Adamas::Compiler::Frontend::NodeKind::Next)
    end
    ...
    it "binds postfix if weaker than infix operators" do
      ...
      Adamas::Compiler::Frontend.node_kind(expr).should eq(Adamas::Compiler::Frontend::NodeKind::Binary)

      # Ensure the RHS is the char literal, not another nested postfix-if expression.
      rhs_id = Adamas::Compiler::Frontend.node_right(expr).not_nil!
      rhs = arena[rhs_id]
      Adamas::Compiler::Frontend.node_kind(rhs).should eq(Adamas::Compiler::Frontend::NodeKind::Char)
    end
