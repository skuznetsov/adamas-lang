...
describe "Adamas::Compiler::Frontend::Parser" do
  describe "Phase 82: Nil-coalescing compound assignment (??=)" do
    ...
    it "parses ??= with instance variable" do
      ...
      target = arena[Adamas::Compiler::Frontend.node_assign_target(assign).not_nil!]
      Adamas::Compiler::Frontend.node_kind(target).should eq(Adamas::Compiler::Frontend::NodeKind::InstanceVar)
    ...
    it "parses ??= with hash/array index" do
      ...
      target = arena[Adamas::Compiler::Frontend.node_assign_target(assign).not_nil!]
      Adamas::Compiler::Frontend.node_kind(target).should eq(Adamas::Compiler::Frontend::NodeKind::Index)
    end
    ...
    it "parses ??= inside method" do
      ...
      Adamas::Compiler::Frontend.node_kind(body).should eq(Adamas::Compiler::Frontend::NodeKind::Assign)
    end
    ...
    it "disambiguates ??= from ?? and ? and =" do
      ...
      Adamas::Compiler::Frontend.node_kind(value1).should eq(Adamas::Compiler::Frontend::NodeKind::Ternary)

      # Second: f ?? g (nil-coalescing)
      assign2 = arena[program.roots[1]]
      value2 = arena[Adamas::Compiler::Frontend.node_assign_value(assign2).not_nil!]
      Adamas::Compiler::Frontend.node_kind(value2).should eq(Adamas::Compiler::Frontend::NodeKind::Binary)
      String.new(Adamas::Compiler::Frontend.node_operator(value2).not_nil!).should eq("??")
      ...
      Adamas::Compiler::Frontend.node_kind(value3).should eq(Adamas::Compiler::Frontend::NodeKind::Binary)
      String.new(Adamas::Compiler::Frontend.node_operator(value3).not_nil!).should eq("??")
      # Left side of ?? should reference 'h'
      left3 = arena[Adamas::Compiler::Frontend.node_left(value3).not_nil!]
      Adamas::Compiler::Frontend.node_kind(left3).should eq(Adamas::Compiler::Frontend::NodeKind::Identifier)
    end
    ...
    it "parses ??= with literal nil on right" do
      ...
      right = arena[Adamas::Compiler::Frontend.node_right(value).not_nil!]
      Adamas::Compiler::Frontend.node_kind(right).should eq(Adamas::Compiler::Frontend::NodeKind::Nil)
    end
  end
end
