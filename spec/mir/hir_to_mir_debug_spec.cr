...
private def with_mir_value_locations(value : String?, &)
  previous = ENV["ADAMAS_ENABLE_MIR_VALUE_LOCATIONS"]?
  if value
    ENV["ADAMAS_ENABLE_MIR_VALUE_LOCATIONS"] = value
  else
    ENV.delete("ADAMAS_ENABLE_MIR_VALUE_LOCATIONS")
  end
  begin
    yield
  ensure
    if previous
      ENV["ADAMAS_ENABLE_MIR_VALUE_LOCATIONS"] = previous
    else
      ENV.delete("ADAMAS_ENABLE_MIR_VALUE_LOCATIONS")
    end
  end
...
describe Adamas::MIR::HIRToMIRLowering do
  describe "debug locations" do
    it "does not retain per-value locations by default" do
      with_mir_value_locations(nil) do
        hir_mod = Adamas::HIR::Module.new("test")
        hir_func = hir_mod.create_function("default_off_debug", Adamas::HIR::TypeRef::INT32)
        block = hir_func.get_block(hir_func.entry_block)
        literal = Adamas::HIR::Literal.new(hir_func.next_value_id, Adamas::HIR::TypeRef::INT32, 7_i64)
        block.add(literal)
        block.terminator = Adamas::HIR::Return.new(literal.id)
        hir_func.record_value_location(literal.id, Adamas::HIR::SourceLocation.new("default_off.cr", 3, 2))

        mir_func = hir_mod.lower_to_mir.functions.find { |f| f.name == "default_off_debug" }.not_nil!
        constant = mir_func.blocks.flat_map(&.instructions).find { |inst| inst.is_a?(Adamas::MIR::Constant) }.not_nil!
        mir_func.value_location(constant.id).should be_nil
      end
    end

    it "preserves index_set source locations on the lowered array_set instruction" do
      with_mir_value_locations("1") do
        hir_mod = Adamas::HIR::Module.new("test")
    ...
    it "restores a pre-existing metadata setting" do
      previous = ENV["ADAMAS_ENABLE_MIR_VALUE_LOCATIONS"]?
      begin
        ENV["ADAMAS_ENABLE_MIR_VALUE_LOCATIONS"] = "sentinel"
        with_mir_value_locations("1") do
          ENV["ADAMAS_ENABLE_MIR_VALUE_LOCATIONS"].should eq("1")
        end
        ENV["ADAMAS_ENABLE_MIR_VALUE_LOCATIONS"].should eq("sentinel")
      ensure
        if previous
          ENV["ADAMAS_ENABLE_MIR_VALUE_LOCATIONS"] = previous
        else
