...
describe Adamas::MIR do
  ...
  describe "AllocationSiteStats" do
    it "calculates average lifetime" do
      stats = Adamas::MIR::AllocationSiteStats.new
    ...
    it "recommends Stack for non-escaping short-lived allocations" do
      ...
      stats.recommended_strategy.should eq(Adamas::MIR::MemoryStrategy::Stack)
    end
    ...
    it "recommends AtomicARC for thread-shared allocations" do
      ...
      stats.recommended_strategy.should eq(Adamas::MIR::MemoryStrategy::AtomicARC)
    end
    ...
    it "recommends Slab for low-escape moderate-lifetime allocations" do
      ...
      stats.recommended_strategy.should eq(Adamas::MIR::MemoryStrategy::Slab)
    end
  ...
  describe "CallSiteStats" do
    it "identifies hot call sites" do
      c = Adamas::MIR::CallSiteStats.new
      c.call_count = 5000
  ...
  describe "ProfileData" do
    it "creates and tracks allocation sites" do
      profile = Adamas::MIR::ProfileData.new
    ...
    it "records escape events" do
      ...
      profile.record_escape(1_u64, Adamas::MIR::EscapeKind::ReturnEscape)
      profile.record_escape(1_u64, Adamas::MIR::EscapeKind::ClosureEscape)
      profile.record_escape(1_u64, Adamas::MIR::EscapeKind::ContainerEscape)
    ...
    it "merges profile data" do
      profile1 = Adamas::MIR::ProfileData.new
      profile1.add_site(1_u64, "func1")
      profile1.record_allocation(1_u64)
      profile1.record_allocation(1_u64)

      profile2 = Adamas::MIR::ProfileData.new
  ...
  describe "ProfileData serialization" do
    it "round-trips through binary format" do
      ...
      # Deserialize
      io.rewind
      loaded = Adamas::MIR::ProfileData.from_binary(io)
    ...
    it "returns nil for invalid magic" do
      ...
      Adamas::MIR::ProfileData.from_binary(io).should be_nil
    end
    ...
    it "round-trips branch statistics" do
      ...
      loaded = Adamas::MIR::ProfileData.from_binary(io).not_nil!
  ...
  describe "CompilerFlags" do
    it "parses --mm=conservative" do
      flags = Adamas::MIR::CompilerFlags.from_args(["--mm=conservative"])
      flags.mm_mode.should eq("conservative")
    end

    it "parses --mm=profile-gen" do
      flags = Adamas::MIR::CompilerFlags.from_args(["--mm=profile-gen"])
      flags.profile_mode.should eq(Adamas::MIR::ProfileMode::Generate)
      flags.should_instrument?.should be_true
      flags.should_use_profile?.should be_false
    end

    it "parses --mm=profile-use" do
      flags = Adamas::MIR::CompilerFlags.from_args(["--mm=profile-use"])
      flags.profile_mode.should eq(Adamas::MIR::ProfileMode::Use)
      flags.should_instrument?.should be_false
      flags.should_use_profile?.should be_true
    end

    it "parses --mm=profile (both)" do
      flags = Adamas::MIR::CompilerFlags.from_args(["--mm=profile"])
      flags.profile_mode.should eq(Adamas::MIR::ProfileMode::Both)
      flags.should_instrument?.should be_true
      flags.should_use_profile?.should be_true
    end

    it "parses --profile-path" do
      flags = Adamas::MIR::CompilerFlags.from_args(["--profile-path=/tmp/my.profile"])
      flags.profile_path.should eq("/tmp/my.profile")
    end

    it "handles multiple flags" do
      flags = Adamas::MIR::CompilerFlags.from_args([
        "--mm=aggressive",
        "--mm=profile-gen",
  ...
  describe "ProfileGuidedOptimizer" do
    it "uses profile data for strategy recommendations" do
      ...
      optimizer = Adamas::MIR::ProfileGuidedOptimizer.new(profile)

      # High escape → GC (default fallback for complex cases)
      recommended1 = optimizer.recommended_strategy(1_u64, Adamas::MIR::MemoryStrategy::GC)
      recommended1.should eq(Adamas::MIR::MemoryStrategy::GC)

      # Low escape, short lifetime → Stack
      recommended2 = optimizer.recommended_strategy(2_u64, Adamas::MIR::MemoryStrategy::GC)
      recommended2.should eq(Adamas::MIR::MemoryStrategy::Stack)
    end
    ...
    it "uses default for insufficient samples" do
      ...
      optimizer = Adamas::MIR::ProfileGuidedOptimizer.new(profile)
      optimizer.recommended_strategy(1_u64, Adamas::MIR::MemoryStrategy::ARC).should eq(Adamas::MIR::MemoryStrategy::ARC)
    end
  ...
  describe "ProfileInstrumentationPass" do
    it "instruments allocations" do
      ...
      builder.alloc(Adamas::MIR::MemoryStrategy::ARC, Adamas::MIR::TypeRef.new(100_u32))
      builder.alloc(Adamas::MIR::MemoryStrategy::GC, Adamas::MIR::TypeRef.new(101_u32))
      builder.ret(nil)
      ...
      block.instructions[0].should be_a(Adamas::MIR::ProfileInstrument)
      block.instructions[1].should be_a(Adamas::MIR::Alloc)
      block.instructions[2].should be_a(Adamas::MIR::ProfileInstrument)
      block.instructions[3].should be_a(Adamas::MIR::Alloc)
    end

    it "registers sites in profile data" do
      mod = Adamas::MIR::Module.new
      func = mod.create_function("my_function", Adamas::MIR::TypeRef::VOID)
      builder = Adamas::MIR::Builder.new(func)

      builder.alloc(Adamas::MIR::MemoryStrategy::Stack, Adamas::MIR::TypeRef::INT32)
      builder.ret(nil)
  ...
  describe "ProfileInstrument" do
    it "represents profile instrumentation call" do
      inst = Adamas::MIR::ProfileInstrument.new(
        0_u32,
        Adamas::MIR::ProfileInstrument::Kind::Alloc,
        12345_u64
      )

      inst.kind.should eq(Adamas::MIR::ProfileInstrument::Kind::Alloc)
      inst.site_id.should eq(12345_u64)
      inst.type.should eq(Adamas::MIR::TypeRef::VOID)
    end

    it "prints correctly" do
      inst = Adamas::MIR::ProfileInstrument.new(
        5_u32,
        Adamas::MIR::ProfileInstrument::Kind::Escape,
        999_u64
      )
