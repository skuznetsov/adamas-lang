...
| T1 ownership/NameId substrate | `PRODUCTION SINGLE-ACTIVE-UNIT HANDOFF COMPLETED / MULTI-ACTIVE-ARENA T1 STILL RED` | One `SemanticIdentityRegistry` owns canonical session-local names and the existing semantic type table; `DefInstanceKey` named arguments are typed `NameId` pairs, and retained semantic key arrays are mutation-safe. Exact admitted explicit-receiver shapes build and validate one private `LocalCallResolution {MethodSymbol, DefInstanceKey}`; `TypeContext` retains that key as lazy analysis output, and an `AstToHir` consumer bound to the same parser arena serializes its exact `DefNode` before the primary legacy overload scorer. When compilation has exactly one active unit, the production semantic prepass analyzes the original arena and hands that context to HIR without a reparse or identity translation; additional parsed `skip_file` units do not weaken that authority. Two or more active units keep the existing shadow aggregate and hand no aggregate identities to HIR. Semantic authority bypasses the legacy self-recursion guess, while one normal call-emission guard fails closed unless both the selected entry and emitted callee still map to that exact `DefNode`. No `ResolutionId`, new registry, mutable HIR certificate, constructor widening, cross-arena handoff, or generic-receiver admission was added. Block, intrinsic, union, index, and other early-return lowering remain outside this bounded ordinary-positional identity claim. The combined semantic CLI suite passes 61 examples, including a two-file recursive-overload case whose extra parsed unit is inactive and a multi-active class-reopening control that preserves the typed recursive HIR body while semantic handoff remains disabled. The semantic compile prepass is opt-in, and the canonical bootstrap control rejects every `ADAMAS_*` setting; therefore this corridor does not participate in B4-F or explain its runtime. |

The production single-unit handoff claim is limited to source-backed call
expressions. Independently generated or macro-expanded nodes remain outside it:
semantic analysis and HIR may append those nodes separately even while sharing
...
the same canonical symbol may be re-exported into a file-local table. Same-arena
