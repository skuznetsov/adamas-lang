...
| L6 | The next generated-stage LLVM-entry movement must be transaction-level, not another local session-edge proxy. | Slice 0k-BX/0k-BY/0k-BZ | `scripts/generated_stage_execution_transaction_report.sh` must join invocation setup, function plan, worker/fallback policy, side-effect contract, tail declaration/stub inputs, output ownership, resource evidence, and B4 final classification under one transaction id for the same produced-compiler run. Current source is measured-red: `STAGE1_COMPILER=bin/adamas REQUIRE_CURRENT_FRONTIER=1 scripts/generated_stage_execution_transaction_report.sh` reports `b4.classification=current_0k_bn_frontier`, `final_classification=abort_unjoined_evidence`, `join_status=phase_local_only`, and `admission_status=rejected_unjoined_evidence`. Negative guard: `REQUIRE_JOINED=1` exits 9 without runtime transaction rows, even for a clean local generated-s2 substitute. Slice 0k-BZ adds the hostile stop rule: adding rows is not enough; joined evidence must select exactly one root-sized transaction-owned old authority edge, explicitly refute `GeneratedStageExecution`, or stop with a named missing runtime row that needs a new SDD slice. Production behavior patches to workers, tail stubs, output, resources, side effects, backend forwarders, memory budgets, `ADAMAS_LLVM_WORKERS=1`, `NamedTuple`/`Tuple` rendering, ambient maps, parser behavior, or `BlockOwner` remain invalid until that joined/root-sized decision exists. | current | [FALSIFIABLE] |
