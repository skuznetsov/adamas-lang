...
| L6 | The next generated-stage LLVM-entry movement must be transaction-level, not another local session-edge proxy. | Slice 0k-BX | Future executable guard: `scripts/generated_stage_execution_transaction_report.sh` must join invocation setup, function plan, worker/fallback policy, side-effect contract, tail declaration/stub inputs, output ownership, resource evidence, and B4 final classification under one transaction id for the same produced-compiler run. Until that report exists and either selects a root-sized transaction-owned old authority edge or refutes the current boundary, production behavior patches to workers, tail stubs, output, resources, side effects, backend forwarders, memory budgets, or `ADAMAS_LLVM_WORKERS=1` are invalid evidence. A report with malformed/unjoined rows must fail closed. | current | [FALSIFIABLE] |
