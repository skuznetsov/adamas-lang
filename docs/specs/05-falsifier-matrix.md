...
| ID | Claim | Source | Smallest Falsifier | Phase | Status |
|----|-------|--------|--------------------|-------|--------|
| O1 | Produced-stage MIR optimization must not read null or uninitialized compiler-runtime `Set`/`Hash` state while validating CopyPropagation local replacements. | Slice 0k-CK/0k-CL/0k-CM/0k-CN/0k-CO/0k-CP/0k-CQ/0k-CR | `REQUIRE_CURRENT_CP_BROAD=1 scripts/hir_block_return_shape_census.sh` builds a temp-source-copy probe and counts block-call wrapper keys with multiple observed return shapes. It currently reports `classification=current_0k_cp_hir_block_return_shape_broad`, `candidate_multi_shape_keys=208`, `candidate_nil_value_coexist_keys=206`, and `candidate_additional_return_shape_bodies=228`, rejecting naive specialization for all untyped `&` helpers. The same run reports a root-sized assigned-tail passthrough discriminator: `assigned_tail_multi_shape_keys=1`, `assigned_tail_additional_return_shape_bodies=4`, and `timed_cp_phase_assigned_tail_passthrough_keys=1`. The next behavior patch must be paired: assigned-tail yield-passthrough classification plus return-shape-specific wrapper materialization. A classifier-only patch or broad return-shape specialization is a falsifier failure. | current | [FRONTIER] |
