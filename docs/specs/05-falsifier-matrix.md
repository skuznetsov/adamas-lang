...
| G6 | The current owner-cache carrier must materialize `Hash(UInt64, BlockOwner)#[]=` as a real body, not an undefined-extern abort stub. | `TODO.md`, LM-ARCH-0K-AW, Slice 0k-CC | `regression_tests/block_owner_index_assign_materialization_repro.sh <compiler>` finds a non-stub self-IR body for the current `BlockOwner` carrier. Slice 0k-CC selects this as the next `MaterializationTransaction` implementation lane: the fix must preserve `BlockOwner` and prove the demanded call-visible setter identity and materialized body identity are the same semantic function before relying on backend emission. | current | [FALSIFIABLE] |
