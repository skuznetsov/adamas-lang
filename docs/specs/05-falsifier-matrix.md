...
| B4 | Produced `s2b` must compile a full-prelude tiny source through LLVM entry without worker-only masking, RSS-kill, or post-`lower_main` crash. | Slice 0k-BN/0k-BO / `00-bootstrap-contract.md` section 4.2 | `scripts/generated_stage_llvm_entry_classifier.sh` builds/accepts stage1 and produced `s2b`, then compiles a full-prelude tiny source with default LLVM workers and `ADAMAS_LLVM_WORKERS=1`. `REQUIRE_CURRENT_FRONTIER=1` currently exits 0 with `classification=current_0k_bn_frontier`; `REQUIRE_CLEAN=1` is the future acceptance gate. | current | [FRONTIER] |
