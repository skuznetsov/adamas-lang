...
| L5 | Generated-stage LLVM emission must have an explicit `LLVMEmissionSession` owner record and a convergence vector before behavior patches to worker policy, side-effect merge, tail stubs, output files, or resource acceptance. | Slice 0k-BQ/0k-BR/0k-BS/0k-BT/0k-BU/0k-BV | `REQUIRE_SESSION=1 REQUIRE_WORKER_PLAN=1 scripts/llvm_emission_session_source_shape_guard.sh` must report `source_shape=session_consumes_function_plan` and `worker_shape=session_consumes_worker_plan` for the consumed `function-list-inline` and `worker-policy-inline` edges. Slice 0k-BT adds a guardrail: a future side-effect, tail, output, or resource slice must prove a downstream consumer reads a vertical `LLVMEmissionSession` contract, not merely that the session gained fields/getters/tags. Slice 0k-BU selects the side-effect source-shape extension. Slice 0k-BV makes it executable and intentionally red on current source: `REQUIRE_SESSION=1 REQUIRE_WORKER_PLAN=1 REQUIRE_SIDE_EFFECT_CONTRACT=1 scripts/llvm_emission_session_source_shape_guard.sh` must report `side_effect_contract_shape=legacy_parallel_side_effect_merge`, nonzero raw writer/merge tag counts, and exit non-zero before 0k-BW; it may pass only when `emit_functions_parallel` delegates both worker side-effect writing and parent side-effect merging through a session-owned `SideEffectMergeContract`. A production behavior change in side-effect merge, tail stubs, output files, or resource acceptance without that consumer source-shape guard is a falsifier. A green source-shape row without the 0k-BV B4/worker/tail/output/resource convergence-vector ledger is also a falsifier. | current | [FALSIFIABLE] |
