...
| L5 | Generated-stage LLVM emission must have an explicit `LLVMEmissionSession` owner record before behavior patches to worker policy, side-effect merge, tail stubs, output files, or resource acceptance. | Slice 0k-BQ/0k-BR/0k-BS | `REQUIRE_SESSION=1 REQUIRE_WORKER_PLAN=1 scripts/llvm_emission_session_source_shape_guard.sh` must report `source_shape=session_consumes_function_plan` and `worker_shape=session_consumes_worker_plan` for the consumed `function-list-inline` and `worker-policy-inline` edges. A future production behavior change in side-effect merge, tail stubs, output files, or resource acceptance without an equivalent session-edge source-shape guard is a falsifier. | current | [FALSIFIABLE] |
