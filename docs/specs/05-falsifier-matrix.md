...
| G6 | The current owner-cache carrier must materialize `Hash(UInt64, BlockOwner)#[]=` as a real body, not an undefined-extern abort stub. | `TODO.md`, LM-ARCH-0K-AW, Slice 0k-CC/0k-CD | `regression_tests/block_owner_index_assign_materialization_repro.sh <compiler>` finds a non-stub self-IR body for the current `BlockOwner` carrier. Slice 0k-CC selects this as the next `MaterializationTransaction` implementation lane. Slice 0k-CD adds a pre-code stop: green G6 is not sufficient unless a `MaterializationTransaction` plan gate first proves the demanded call-visible setter identity, selected semantic identity, materialized body identity, HIR/MIR body presence, and backend-visible body are the same semantic function. Backend rescue, forwarders, requested-name forcing, broad `NamedTuple`/`Tuple` rendering, global ambient-map policy changes, parser changes, and `BlockOwner` rollback are invalid ways to satisfy this row. | current | [FALSIFIABLE] |
