...
| B4 | Produced `s2b` must compile a full-prelude tiny source through LLVM entry without worker-only masking, RSS-kill, or post-`lower_main` crash. | Slice 0k-BN / `00-bootstrap-contract.md` section 4.2 | Build fresh stage1, build fresh `s2b` through `scripts/run_safe.sh`, then compile a full-prelude `puts 42`/hello source with the produced compiler twice: default LLVM workers and `ADAMAS_LLVM_WORKERS=1`. Current evidence: default workers report `parallel emission failed: Invalid bound for rand: 0` then hit the 4096MB RSS limit; `ADAMAS_LLVM_WORKERS=1` removes that symptom but still exits 139 after `pass3 after lower_main call`. | current | [FRONTIER] |
