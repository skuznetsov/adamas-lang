...
| ID | Claim | Source | Smallest Falsifier | Phase | Status |
|----|-------|--------|--------------------|-------|--------|
| O1 | Produced-stage MIR optimization must not read null or uninitialized compiler-runtime `Set`/`Hash` state while validating CopyPropagation local replacements. | Slice 0k-CK/0k-CL/0k-CM/0k-CN/0k-CO/0k-CP/0k-CQ | `scripts/hir_block_return_shape_census.sh` builds a temp-source-copy probe and counts block-call wrapper keys with multiple observed return shapes. It currently reports `classification=current_0k_cp_hir_block_return_shape_broad`, `candidate_multi_shape_keys=208`, `candidate_nil_value_coexist_keys=206`, and `candidate_additional_return_shape_bodies=228`. The `timed_cp_phase` row still has nil plus `Int32` / `Set(UInt32)` / `Nil | DominanceInfo` / `Hash(UInt32, Int32)` shapes, but naive return-shape specialization for all untyped `&` helpers is rejected as broad. The next falsifier must distinguish true return-demanded yield-passthrough helpers from ordinary iterators/scope helpers before any behavior patch. | current | [FRONTIER] |
