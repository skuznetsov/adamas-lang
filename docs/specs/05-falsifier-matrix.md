...
| L5 | Generated-stage LLVM emission must have an explicit `LLVMEmissionSession` owner record before behavior patches to worker policy, side-effect merge, tail stubs, output files, or resource acceptance. | Slice 0k-BQ | Add a source-shape guard proving at least one old authority edge (`function-list-inline`, `worker-policy-inline`, `side-effect-field-bag`, `tail-stub-from-mutable-sets`, or `output-file-owned-by-cli`) is consumed by `LLVMEmissionSession`. A production behavior change in these areas without that guard is a falsifier. | current | [MISSING-FALSIFIER] |
