...
| ID | Claim | Source | Smallest Falsifier | Phase | Status |
|----|-------|--------|--------------------|-------|--------|
| O1 | Produced-stage MIR optimization must not read null or uninitialized compiler-runtime `Set`/`Hash` state while validating CopyPropagation local replacements. | Slice 0k-CK/0k-CL/0k-CM | `REQUIRE_CURRENT_CM=1 scripts/mir_timed_phase_return_frontier_classifier.sh` reuses O1, runs a `can_skip_dominators_for_local_replacements?` entry probe, and disassembles `timed_cp_phase`, `apply_replacements`, and the collect block. It currently reports `classification=current_0k_cm_timed_cp_phase_block_return_frontier`, `can_skip_affected_block_ids_x3_zero=1`, `timed_zeroes_after_block=1`, `timed_enabled_branch_returns_zero=1`, `apply_collect_result_is_nil_void=1`, and `collect_block_returns_set_slot=1`. The next classifier must name the compiler-source seam that materializes `timed_cp_phase(String, &)` as a void/null-return block wrapper, or refute this boundary before any behavior patch. | current | [FRONTIER] |
