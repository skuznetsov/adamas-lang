...
| `PhaseAuthority` / `GeneratedStageExecution` | Post-0k-DL evidence is joined, resource-classified, and phase-split, but still not behavior-admissible. `STAGE1_COMPILER=/tmp/adamas_phase_stage1 TAIL_LINES=30 REQUIRE_JOINED=1 REQUIRE_POST_CU_RESOURCE=1 REQUIRE_RESOURCE_PHASE_SPLIT=1 scripts/generated_stage_execution_transaction_report.sh` exits 0 with `final_classification=abort_resource_after_lower_main`, `join_status=joined`, `resource.default_memory_kill=1`, `resource.workers1_memory_kill=1`, `output.commit_record=llvm_ir_started_without_commit:file`, `resource.llvm_generate_last_phase=function_emission_start`, `resource.llvm_generate_phase_split=during_function_emission`, and `runtime.llvm_generate_phase_rows=2`. | A production slice on this lane must now name a transaction-owned function-emission edge and reduce ambiguity inside `LLVMIRGenerator#generate`: function-plan memory growth, sequential/parallel function chunk emission, worker/parent output buffering, or another measured function-emission consumer before behavior changes. | Memory-budget acceptance, forced `ADAMAS_LLVM_WORKERS=1`, worker/rand patches, output-file behavior changes, tail stubs, metadata/DWARF/type-name patches, backend semantic changes, or treating `during_function_emission` as bootstrap progress without a root-sized transaction consumer. |
...
| L6 | `PhaseAuthority` / `GeneratedStageExecution` | active joined-transaction resource pressure after 0k-DL; report surface remains guard-only until a root-sized transaction-owned function-emission consumer is selected | fresh 0k-DL evidence reports `join_status=joined`, `final_classification=abort_resource_after_lower_main`, `resource.default_memory_kill=1`, `resource.workers1_memory_kill=1`, `output.commit_record=llvm_ir_started_without_commit:file`, `resource.llvm_generate_phase_split=during_function_emission`, and `admission_status=rejected_no_root_sized_consumer` |
