...
| `PhaseAuthority` / `GeneratedStageExecution` | Post-0k-DM evidence is joined, resource-classified, phase-split, and function-emission-split, but still not behavior-admissible. `STAGE1_COMPILER=/tmp/adamas_function_phase_stage1 TAIL_LINES=30 REQUIRE_JOINED=1 REQUIRE_POST_CU_RESOURCE=1 REQUIRE_RESOURCE_PHASE_SPLIT=1 REQUIRE_FUNCTION_EMISSION_SPLIT=1 scripts/generated_stage_execution_transaction_report.sh` exits 0 with `final_classification=abort_resource_after_lower_main`, `resource.function_emission_split=during_sequential_function_emit`, `resource.function_emission_last_phase=sequential_progress`, `resource.function_emission_last_index=80`, `resource.function_emission_last_total=150`, `resource.function_emission_mode_join_status=default_only`, `runtime.default_function_emission_phase_rows=13`, and `runtime.workers1_function_emission_phase_rows=0`. | A production slice on this lane must now either make the workers=1 function-emission observability gap root-sized, or name a transaction-owned default-mode sequential function-emission edge such as emitted output growth, per-function memory growth, function-plan/chunk shape, or another measured consumer before behavior changes. | Memory-budget acceptance, forced `ADAMAS_LLVM_WORKERS=1`, worker/rand patches, output-file behavior changes, tail stubs, metadata/DWARF/type-name patches, backend semantic changes, or treating default-mode `sequential_progress index=80/150` as green bootstrap without a root-sized transaction consumer and workers=1 residual statement. |
...
| L6 | `PhaseAuthority` / `GeneratedStageExecution` | active joined-transaction resource pressure after 0k-DM; report surface remains guard-only until a root-sized transaction-owned function-emission consumer is selected | fresh 0k-DM evidence reports `join_status=joined`, `final_classification=abort_resource_after_lower_main`, `resource.function_emission_split=during_sequential_function_emit`, `resource.function_emission_last_index=80`, `resource.function_emission_last_total=150`, `resource.function_emission_mode_join_status=default_only`, `runtime.default_function_emission_phase_rows=13`, `runtime.workers1_function_emission_phase_rows=0`, and `admission_status=rejected_no_root_sized_consumer` |
...
| `GeneratedStageExecution` / `LLVMEmissionSession` | 0k-BN..0k-CJ established the generated-stage owner spine and behavior-neutral transaction/output/session contracts; 0k-CU consumed the old O1 `affected_block_ids` frontier; 0k-DK/0k-DL/0k-DM remeasure the resulting resource residual through joined transaction, LLVM-generate phase, and function-emission phase rows. Current strict evidence reports `final_classification=abort_resource_after_lower_main`, `resource.function_emission_split=during_sequential_function_emit`, `resource.function_emission_last_index=80`, `resource.function_emission_last_total=150`, `resource.function_emission_mode_join_status=default_only`, and `runtime.workers1_function_emission_phase_rows=0`. | The next production movement must either make the workers=1 function-emission observability gap root-sized, or select a transaction-owned default-mode sequential emission edge such as emitted output growth, per-function memory growth, function-plan/chunk shape, or another measured consumer. More behavior-neutral report rows are invalid unless they retire/refute this selector or reduce owner-spine ambiguity. | Patching `emit_functions_parallel` because of the rand symptom; raising `run_safe` memory as acceptance evidence; forcing `ADAMAS_LLVM_WORKERS=1` as a fix; treating green `SideEffectMergeContract` source shape, transaction-report presence, joined evidence, green G6 availability, 0k-CH docs, 0k-CJ output-owner source shape, 0k-DK resource classification, 0k-DL phase split, or 0k-DM function-emission split as green `s2b`/`s3b`; selecting `TailDeclarationPlan`, `OutputOwnership`, `ResourceEvidence`, metadata/DWARF/type-name, worker policy, memory-budget acceptance, or backend semantics while the current split is `during_sequential_function_emit`; adding another classifier that only narrows a crash offset without naming a transaction fact and owner spine; adding a report-only selector that does not retire/refute an older report surface; changing side-effect merge semantics, tail missing-body stubs, undefined-extern rescue, output files, or resource gates without an owner-spine decision. |
