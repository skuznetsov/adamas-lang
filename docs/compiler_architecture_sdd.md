...
| `MaterializationTransaction` / exact body availability | Current active selector after 0k-DB. G6 `BlockOwner` setter availability is guard-green. The post-lowering completion classifier reaches `[MAT_TX]`, `[MAT_DONE]`, and `[MAT_EMIT]`; terminal fields on `[MAT_DONE]` select one still-broad cause: `attempt_lowering_returned_no_hir_function`, 14 rows across 9 groups. Separate `MaterializationAttemptResult` / consumer-ledger surfaces are refuted for this path because they stopped generated-stage runs before backend emission. | Next executable receipt: producer-boundary split for exact `body_symbol` materialization attempts that return from lowering without registering the exact materialized HIR function. The receipt must name which lowering branch returns no function, keep the existing `[MAT_DONE]` terminal fields as the authority, and stop if the selected branch is broad or ambiguous. | Backend undefined-extern rescue, forwarder rescue, requested-name forcing, per-method Array/Slice/IO/Atomic/String::Builder/Int32 patches, broad `NamedTuple`/`Tuple` rendering, global ambient-map policy, `BlockOwner` rollback, treating `created_body` as proof of a present HIR body, treating pre-lowering `InProgress` as a completed-body root, or adding another result/consumer layer that does not first pass generated-stage emission. |
