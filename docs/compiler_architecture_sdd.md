...
| G6 | `MaterializationTransaction` | pre-code plan complete; admitted implementation lane is a transaction-owned availability proof | current falsifier exists and the failure is a body-materialization authority edge for the current `BlockOwner` carrier. 0k-CE pins the setter producer, materialization owner, HIR/MIR/backend consumers, invariant, and stop rules before any production source edit |
