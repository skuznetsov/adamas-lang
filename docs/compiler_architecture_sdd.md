...
| `CodePathStatus` | `cli.metrics.identity_dry_run` and `cli.metrics.phase0_metrics` are classified as `debug_only`, not `delete_ready`, on a fresh stage1 no-prelude cleanup preflight. | Select a different named path for cleanup/delete, or extend the cleanup selector to enumerate a root-sized candidate with default-behavior, HIR/MIR/LLVM, bootstrap, and protecting-falsifier evidence. | Deleting `identity_dry_run` or `phase0_metrics`; deleting debug/probe/fallback paths from static grep output; using runtime liveness as semantic ownership evidence. |
