...
| `MaterializationTransaction` / exact body availability | Current active selector after 0k-DF. G6 `BlockOwner` setter availability is guard-green. The temp-source `lower_method` terminal classifier preserves `[MAT_EMIT]` reachability and splits the current residual into `no_exact_no_call` (6 rows), `abstract_method` (4 rows), `no_exact_matching_full_name_without_exit` (3 rows), and `completed_method` (1 row), across 12 root-sized groups. The broad class has zero exact/requested `[MAT_METHOD_CALL]` rows, while abstract controls join to call rows. This is still mixed/broad, so no behavior patch is admitted. Separate `MaterializationAttemptResult` / consumer-ledger surfaces and production `lower_method` trace-object plumbing are refuted for this path because they stopped generated-stage runs before backend emission. | Next executable receipt: split the pre-call gap between transaction logging and actual `lower_method` invocation, covering type-param isolation, namespace override, arity fallback, and any pre-call return/skip edge while preserving generated-stage `[MAT_EMIT]` reachability. Stop if the selected branch remains broad or ambiguous. | Backend undefined-extern rescue, forwarder rescue, requested-name forcing, per-method Array/Slice/IO/Atomic/String::Builder/Int32 patches, broad `NamedTuple`/`Tuple` rendering, global ambient-map policy, `BlockOwner` rollback, treating `created_body` as proof of a present HIR body, treating pre-lowering `InProgress` as a completed-body root, adding another result/consumer layer that does not first pass generated-stage emission, or threading a trace object through production `lower_method`. |
