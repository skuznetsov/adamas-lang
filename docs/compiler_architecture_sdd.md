...
| Enum case-equality semantic owner through late repair | Initial `===` lowering and late receiver repair share the retained per-function enum-value sidecar as the semantic receiver-owner proof. Normal repair dispatches through that enum owner while retaining the original integer receiver value, descriptor, and wrapper receiver ABI. Cross-owner and same-owner-bodyful falsifiers poison an exact `CaseKind` call with `OtherKind#===$CaseKind` and `CaseKind#===$Nil | CaseKind`; repair restores the exact target and proves receiver/argument ABI. A nilable-left negative guard rejects scalar enum ownership. Exact/nilable targets remain `Bool`, non-virtual, blockless, and physically carrier-backed. No preservation bypass, new registry, cast, or name allowlist is admitted. The full HIR suite is 377/0 with two existing pending; the durable oracle and fresh reducer HIR/MIR/LLVM/link/runtime are green. | Closes the classified 137-occurrence function-body enum-owner defect only. At the same `queue@iter2` boundary, `Object#===` falls to zero and is conserved as `NodeKind#===` 78, `Token::Kind#===` 47, and `DWARF::AT#===` 12; `Hash#==` is 55, functions 12854, and unique missing targets increase 2048 -> 2052. Top-level `__adamas_main` enum case ownership remains a separate observed frontier. Therefore no B4-F closure, global fanout reduction, timing admission, fresh stage2, or produced-stage smoke is claimed. |
