...
| `MaterializationTransaction` / exact body availability | Current active selector after 0k-DE. G6 `BlockOwner` setter availability is guard-green. The temp-source `lower_method` terminal classifier preserves `[MAT_EMIT]` reachability and splits the current residual into `no_exact_no_entry` (6 rows), `abstract_method` (4 rows), `no_exact_matching_full_name_without_exit` (3 rows), and `completed_method` (1 row), across 12 root-sized groups. This is still mixed/broad, so no behavior patch is admitted. Separate `MaterializationAttemptResult` / consumer-ledger surfaces and production `lower_method` trace-object plumbing are refuted for this path because they stopped generated-stage runs before backend emission. | Next executable receipt: split the broad `no_exact_no_entry` class by actual lower_method call input, selected DefNode/source owner, and requested/target/body symbol relation while preserving generated-stage `[MAT_EMIT]` reachability. Stop if the selected branch remains broad or ambiguous. | Backend undefined-extern rescue, forwarder rescue, requested-name forcing, per-method Array/Slice/IO/Atomic/String::Builder/Int32 patches, broad `NamedTuple`/`Tuple` rendering, global ambient-map policy, `BlockOwner` rollback, treating `created_body` as proof of a present HIR body, treating pre-lowering `InProgress` as a completed-body root, adding another result/consumer layer that does not first pass generated-stage emission, or threading a trace object through production `lower_method`. |
