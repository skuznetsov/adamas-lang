...
| `CodePathStatus` | Runtime cleanup inventory now reports 26 no-prelude CLI paths and `inventory_delete_ready_rows=0`. `identity_dry_run` and `phase0_metrics` remain `debug_only`, not `delete_ready`. | Select one `not_taken_unproven` path and add a protecting falsifier before any deletion, or extend the selector with a real `eligible_delete_ready_candidate` class. | Deleting `identity_dry_run` or `phase0_metrics`; deleting any `not_taken_unproven` path from inventory alone; deleting debug/probe/fallback paths from static grep output; using runtime liveness as semantic ownership evidence. |
