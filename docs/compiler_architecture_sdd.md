...
| `MaterializationTransaction` / exact body availability | Current active selector after 0k-DA. G6 `BlockOwner` setter availability is guard-green. The post-lowering completion classifier reaches `[MAT_TX]`, `[MAT_DONE]`, and `[MAT_EMIT]` and reports one broad cause: `lowering_completed_without_hir_function`, 14 rows across 9 groups. | Next executable receipt: behavior-neutral `MaterializationAttemptResult` / terminal-status owner for exact `body_symbol` materialization attempts. It must record why a materialization attempt finished without a HIR function body, split the broad completion class by named terminal reason, and name the consumer edge that will eventually stop falling through to `ExternCall` without an owned availability result. It must stop if the selected terminal class is broad or ambiguous. | Backend undefined-extern rescue, forwarder rescue, requested-name forcing, per-method Array/Slice/IO/Atomic/String::Builder/Int32 patches, broad `NamedTuple`/`Tuple` rendering, global ambient-map policy, `BlockOwner` rollback, treating `created_body` as proof of a present HIR body, treating pre-lowering `InProgress` as a completed-body root, or adding another report-only layer without a terminal-status owner. |
