...
| `MaterializationTransaction` / exact body availability | Current active selector after 0k-DD. G6 `BlockOwner` setter availability is guard-green. The temp-source `lower_method` terminal classifier preserves `[MAT_EMIT]` reachability and splits the current residual into `no_exact_method_exit` (9 rows), `abstract_method` (4 rows), and `created_hir_function` (1 row), across 9 root-sized groups. This is still mixed/broad, so no behavior patch is admitted. Separate `MaterializationAttemptResult` / consumer-ledger surfaces and production `lower_method` trace-object plumbing are refuted for this path because they stopped generated-stage runs before backend emission. | Next executable receipt: pick one terminal class, likely broad `no_exact_method_exit`, and split it by source identity, resolved DefNode, and full-name derivation while preserving generated-stage `[MAT_EMIT]` reachability. Stop if the selected branch remains broad or ambiguous. | Backend undefined-extern rescue, forwarder rescue, requested-name forcing, per-method Array/Slice/IO/Atomic/String::Builder/Int32 patches, broad `NamedTuple`/`Tuple` rendering, global ambient-map policy, `BlockOwner` rollback, treating `created_body` as proof of a present HIR body, treating pre-lowering `InProgress` as a completed-body root, adding another result/consumer layer that does not first pass generated-stage emission, or threading a trace object through production `lower_method`. |
