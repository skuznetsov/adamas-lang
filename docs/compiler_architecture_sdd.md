...
| `MaterializationTransaction` / exact body availability | Current active selector after 0k-DC. G6 `BlockOwner` setter availability is guard-green. The post-lowering completion classifier reaches `[MAT_TX]`, `[MAT_DONE]`, and `[MAT_EMIT]`; terminal fields on `[MAT_DONE]` now select one still-broad producer class: `attempt_lowering_returned_no_hir_function__producer_instance_class_info_lower_method__created_none`, 14 rows across 9 root-sized groups. Separate `MaterializationAttemptResult` / consumer-ledger surfaces and optional `lower_method` trace-object plumbing are refuted for this path because they stopped generated-stage runs before backend emission. | Next executable receipt: split the `instance_class_info_lower_method` / `created_none` producer class inside or immediately around `lower_method`, while keeping `[MAT_DONE]` as the authority and preserving generated-stage `[MAT_EMIT]` reachability. Prefer a temp-source classifier or another self-host-safe field. Stop if the selected branch remains broad or ambiguous. | Backend undefined-extern rescue, forwarder rescue, requested-name forcing, per-method Array/Slice/IO/Atomic/String::Builder/Int32 patches, broad `NamedTuple`/`Tuple` rendering, global ambient-map policy, `BlockOwner` rollback, treating `created_body` as proof of a present HIR body, treating pre-lowering `InProgress` as a completed-body root, adding another result/consumer layer that does not first pass generated-stage emission, or threading a trace object through `lower_method`. |
