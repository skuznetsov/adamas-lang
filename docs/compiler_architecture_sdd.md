...
| `PhaseAuthority` / `GeneratedStageExecution` | Post-0k-CU remeasurement is joined and resource-classified, but still not behavior-admissible. `STAGE1_COMPILER=/tmp/adamas_postcu_stage1 TAIL_LINES=30 REQUIRE_JOINED=1 REQUIRE_POST_CU_RESOURCE=1 scripts/generated_stage_execution_transaction_report.sh` exits 0 with `final_classification=abort_resource_after_lower_main`, `join_status=joined`, `resource.default_memory_kill=1`, `resource.workers1_memory_kill=1`, `resource.workers1_exit139=0`, `output.commit_record=llvm_ir_started_without_commit:file`, and `tail.semantic_vs_input_split=tail_not_reached_after_output_start`. | A production slice on this lane must name the old authority edge behind the post-`lower_main` resource corridor and reduce owner-spine ambiguity: e.g. split `output.llvm_ir_start -> output.llvm_ir_written/binary_compile_result`, LLVM session function-plan memory growth, or another transaction-owned resource consumer before behavior changes. | Memory-budget acceptance, forced `ADAMAS_LLVM_WORKERS=1`, worker/rand patches, output-file behavior changes, tail stubs, backend emission changes, or treating `abort_resource_after_lower_main` as bootstrap progress without a root-sized transaction consumer. |
...
| L6 | `PhaseAuthority` / `GeneratedStageExecution` | active joined-transaction resource pressure after 0k-DK; report surface remains guard-only until a root-sized transaction-owned resource/output/function-plan consumer is selected | fresh 0k-DK evidence reports `join_status=joined`, `final_classification=abort_resource_after_lower_main`, `resource.default_memory_kill=1`, `resource.workers1_memory_kill=1`, `output.commit_record=llvm_ir_started_without_commit:file`, and `admission_status=rejected_no_root_sized_consumer` |
